...
/**
 * {@link JsonGenerator} that outputs Erlang map respresenting JSON content
 * (i.e. as if produced by talentdeficit/jsx:decode with return_maps option)
 * using a {@link java.io.Writer} which handles character encoding.
 */
public class WriterBasedErlangJsonGenerator
    ...
    protected char _lessThanChar = '<';
    protected char _greaterThanChar = '>';
    ...
    protected final void _writeFieldName(String name, boolean commaBefore) throws IOException {
        if (_cfgPrettyPrinter != null) {
            ...
            _reportError("null PrettyPrinter for Erlang");
        }
    }
    ...
    private void _writeShort(short s) throws IOException {
    ...
    private void _writeLong(long l) throws IOException {
    ...
    private void _writeOpenQuote() throws IOException
    {
        if ((_outputTail + 3) >= _outputEnd) {
            _flushBuffer();
        }
        _outputBuffer[_outputTail++] = _lessThanChar;
        _outputBuffer[_outputTail++] = _lessThanChar;
        _outputBuffer[_outputTail++] = _quoteChar;
    }

    private void _writeCloseQuote() throws IOException
    {
        if ((_outputTail + 3) >= _outputEnd) {
            _flushBuffer();
        }
        _outputBuffer[_outputTail++] = _quoteChar;
        _outputBuffer[_outputTail++] = _greaterThanChar;
        _outputBuffer[_outputTail++] = _greaterThanChar;
    }
